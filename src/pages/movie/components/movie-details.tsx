...
export const MovieDetails = ({
  onVibeClick,
}: MovieDetailsProps): JSX.Element | null => {
  const { detailsMovie, setMovieDetails } = useMovieStore();
  ...
  useEffect(() => {
    if (id) {
      ...
      getMovieDetails(movieId)
        ...
        .catch((error) =>
          console.error("Error fetching detalles de pelicula:", error)
        );
    }
  }, [id, setMovieDetails]);
    ...
    <section className="conteiner-details-movie">
      ...
      <section className="details-section">
        ...
        <div className="movie-info">
          <strong className="country-movie">{origin_country || "N/A"}</strong>
        ...
        <button className="btn-1-reserve" onClick={handleReserve}>
          Reservar asiento
        </button>
      </section>
    </section>
